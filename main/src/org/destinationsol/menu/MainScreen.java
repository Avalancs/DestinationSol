...
public class MainScreen implements SolUiScreen {
  ...
  public MainScreen(MenuLayout menuLayout, TextureManager textureManager, boolean mobile, float r, GameOptions gameOptions) {
    ...
    myTutCtrl = new SolUiControl(menuLayout.buttonRectangleForRow(4), true, Input.Keys.T);
    myTutCtrl.setDisplayName("Tutorial");
    myControls.add(myTutCtrl);

    myNewGameCtrl = new SolUiControl(menuLayout.buttonRectangleForRow(3), true, gameOptions.getKeyShoot());
    myNewGameCtrl.setDisplayName("New Game");
    myControls.add(myNewGameCtrl);

    myOptionsCtrl = new SolUiControl(mobile ? null : menuLayout.buttonRectangleForRow(2), true, Input.Keys.O);
    myOptionsCtrl.setDisplayName("Options");
    myControls.add(myOptionsCtrl);

    myExitCtrl = new SolUiControl(menuLayout.buttonRectangleForRow(1), true, gameOptions.getKeyEscape());
